...
// This prevents creating too many points where runs run parallel
...
interface RunIntersection {
  ...
  coords: number[][];
  segmentIndex: number; // The segment where intersection occurs
  pointOnRun: [number, number, number]; // Exact intersection point
  intersectingRunId: string; // The other run that intersects here
}
...
export function buildNavigationGraph(
  ...
  // ========================================================================
  // Create connection nodes at run intersections and near lift endpoints
  // This enables flexible routing that follows pistes rather than walking
  // ========================================================================
  ...
  for (const run of skiArea.runs) {
    ...
    runDataList.push({
      run,
      orderedCoords,
      startNodeId: `run-${run.id}-start`,
      endNodeId: `run-${run.id}-end`,
    });
  }

  // Track intersection points per run to avoid duplicates
      ...
      for (let segA = 0; segA < runA.orderedCoords.length - 1; segA++) {
        ...
        for (let segB = 0; segB < runB.orderedCoords.length - 1; segB++) {
          ...
          if (dist < RUN_INTERSECTION_DISTANCE) {
            ...
            const intersectionA: RunIntersection = {
              runId: runA.run.id,
              runName: runA.run.name,
              runDifficulty: runA.run.difficulty,
              coords: runA.orderedCoords,
              segmentIndex: segA,
              pointOnRun: [closestA.lng, closestA.lat, closestA.elev],
              intersectingRunId: runB.run.id,
            };

            const intersectionB: RunIntersection = {
              runId: runB.run.id,
              runName: runB.run.name,
              runDifficulty: runB.run.difficulty,
              coords: runB.orderedCoords,
              segmentIndex: segB,
              pointOnRun: [closestB.lng, closestB.lat, closestB.elev],
              intersectingRunId: runA.run.id,
            };

            // Add to intersection lists (checking for minimum spacing)
            for (const intersection of [intersectionA, intersectionB]) {
              ...
              for (const existing of existingIntersections) {
                ...
                if (distBetween < MIN_INTERSECTION_SPACING) {
                  tooClose = true;
                  break;
                }
              }
  ...
  // Also find points on runs near lift endpoints
  const liftEndpoints: { lng: number; lat: number; elev: number; liftId: string; type: 'start' | 'end' }[] = [];
  for (const lift of skiArea.lifts) {
    if (!lift.geometry || lift.geometry.type !== 'LineString') continue;
    const coords = lift.geometry.coordinates;
    if (coords.length < 2) continue;
    ...
    liftEndpoints.push({
      ...
      elev: (coords[coords.length - 1][2] as number) || 0,
      liftId: lift.id,
      type: 'end',
    });
  }

  // Find runs near lift endpoints
  for (const liftEndpoint of liftEndpoints) {
    for (const runData of runDataList) {
      for (let seg = 0; seg < runData.orderedCoords.length - 1; seg++) {
        ...
        const dist = haversineDistance(liftEndpoint.lat, liftEndpoint.lng, closest.lat, closest.lng);

        if (dist < LIFT_ENDPOINT_SNAP_DISTANCE) {
          // Check if this point is far enough from existing intersections
          const existingIntersections = runIntersections.get(runData.run.id) || [];
          let tooClose = false;
          ...
          for (const existing of existingIntersections) {
            ...
            if (distBetween < MIN_INTERSECTION_SPACING / 2) { // Use smaller spacing for lift connections
              tooClose = true;
              break;
            }
          }
          ...
          if (!tooClose) {
            existingIntersections.push({
              runId: runData.run.id,
              runName: runData.run.name,
              runDifficulty: runData.run.difficulty,
              coords: runData.orderedCoords,
              segmentIndex: seg,
              pointOnRun: [closest.lng, closest.lat, closest.elev],
              intersectingRunId: `lift-${liftEndpoint.liftId}`,
            });
            runIntersections.set(runData.run.id, existingIntersections);
          }
        }
      }
    }
  ...
  // Sort intersections by segment index (descending) so we can split from end to start
  for (const [runId, intersections] of runIntersections) {
    if (intersections.length === 0) continue;

    // Sort by segment index (descending) to process from end to start
    intersections.sort((a, b) => b.segmentIndex - a.segmentIndex);
    ...
    for (let i = intersections.length - 1; i >= 0; i--) { // Process from start to end
      ...
      if (coordsToSplit.length >= 2) {
        ...
        const edgeToSplit: NavigationEdge = {
          id: `edge-run-${runId}-seg-${currentSegmentStart}-to-${intersection.segmentIndex}`,
    ...
    const coordsToEnd = [
      intersections[0].pointOnRun, // Last processed intersection
    ...
    if (coordsToEnd.length >= 2) {
      ...
      const edgeToEnd: NavigationEdge = {
        id: `edge-run-${runId}-seg-${currentSegmentStart}-to-end`,
        fromNodeId: currentFromNode,
