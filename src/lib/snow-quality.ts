...
// Condition metadata
const CONDITION_INFO: Record<SnowCondition, { icon: string; label: string; color: string }> = {
...
function calculateAltitude(geometry: RunData['geometry']): { min: number; max: number } | undefined {
...
function calculateSteepness(geometry: RunData['geometry']): number | undefined {
  ...
  for (let i = 1; i < coords.length; i++) {
    ...
    const dLon = (curr[0] - prev[0]) * 111000 * Math.cos(prev[1] * Math.PI / 180);
...
export function calculateSnowQuality(
  ...
  if (currentWeather && currentWeather.windSpeed > 30) {
    ...
    factors.push({
      name: 'High Wind',
      impact: 'negative',
      description: `${Math.round(currentWeather.windSpeed)} km/h affecting snow surface`,
    });
  }
  ...
  const condition = determineCondition(
    score, currentTemp, maxTemp, minTemp, daysSinceSnow, 
    recentSnowfall, steepness, isAfternoon, sunFacing, sunUp,
  ...
  return {
    ...
    quality: {
      ...
      condition,
      confidence: calculateConfidence(hourlyWeather.length, dailyWeather.length),
      factors,
...
function generateDescription(condition: SnowCondition, score: number, factors: SnowFactor[]): string {
  const conditionLabel = CONDITION_INFO[condition].label;
  const qualityWord = score >= 8 ? 'Excellent' : score >= 6 ? 'Good' : score >= 4 ? 'Fair' : 'Poor';
...
export function calculateResortSnowSummary(
  ...
  const avgScore = analyses.reduce((sum, a) => sum + a.quality.score, 0) / analyses.length;
  
  // Count conditions
  const conditionCounts: Record<string, number> = {};
  analyses.forEach(a => {
    conditionCounts[a.quality.condition] = (conditionCounts[a.quality.condition] || 0) + 1;
  });
  ...
  if (conditionBreakdown.some(c => c.condition === 'powder' && c.percentage > 20)) {
    recommendations.push('Fresh powder on some runs - get there early!');
  }
  if (conditionBreakdown.some(c => c.condition === 'slush' && c.percentage > 30)) {
    recommendations.push('Stick to higher altitudes and north-facing slopes.');
  }
  if (conditionBreakdown.some(c => c.condition === 'icy' && c.percentage > 30)) {
    recommendations.push('Edges sharpened? Careful on icy patches.');
  }
  if (conditionBreakdown.some(c => c.condition === 'moguls' && c.percentage > 20)) {
...
export function analyzeResortSnowQuality(
  ...
  const analyses = runs.map(run => 
    calculateSnowQuality(run, currentTime, hourlyWeather, dailyWeather, sunAzimuth, sunAltitude)
  );
